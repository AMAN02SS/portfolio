...
useEffect(() => {
  ...
  return () => observer.disconnect();
    ...
    return (
        <div>
                <div className='text-center text-white py-0 md:py-15 my-0 md:my-10' ref={ref}>
                    <div className={`${BricolageGrotesque.className} text-3xl md:text-5xl leading-15`}>
                    ...
                    <h1 className={`${LavishlyYours.className} text-[#45ff8f] text-3xl md:text-6xl transition-all duration-1000 ease-out will-change-[opacity,filter,transform] ${visible ? "opacity-100 blur-0 translate-y-0" : "opacity-0 blur-md translate-x-4"} delay-3000`}>in the everyday</h1>
                </div>
        </div>
    )
}
